...
exports.postOrder = async (req, res) => {
  try {
    ...
    if (!customerEmail) {
      return res.status(400).json({ error: "Customer email is required to send confirmation." });
    }
    ...
    const orderDetails = `
      ...
      <ul>
        ${cartItems.map(item => `<li>${item.productName} - ${item.quantity} x $${item.selectedVariant?.price}</li>`).join("")}
      </ul>
    ...
    res.status(500).json({ error: error.message,data:process.env.SMTP_USER });
  }
};
